import { Bell, CheckCheck, ArrowLeftRight, Gift, Pickaxe, Users, Info, Trash2, X } from "lucide-react";
...
const NotificationItem = ({
  ...
  return (
    <div
      className={`w-full text-left p-3 flex gap-3 items-start border-b border-border/50 last:border-0 ${
        !notification.is_read ? "bg-primary/5" : ""
      }`}
    >
      <button
        className="flex gap-3 items-start flex-1 min-w-0 hover:opacity-80 transition-opacity"
        onClick={() => {
      ...
      <button
        className="mt-1 p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors flex-shrink-0"
        onClick={(e) => {
          e.stopPropagation();
