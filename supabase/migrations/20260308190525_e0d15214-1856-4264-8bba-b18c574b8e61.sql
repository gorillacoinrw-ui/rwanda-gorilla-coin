...
BEGIN
  INSERT INTO public.profiles (user_id, display_name, referral_code, phone)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'display_name', 'Gorilla Miner'),
  ...
  IF ref_code IS NOT NULL AND ref_code != '' THEN
    ...
    IF referrer_user_id IS NOT NULL AND referrer_user_id != NEW.id THEN
      ...
      UPDATE public.profiles SET coin_balance = coin_balance + 15 WHERE user_id = referrer_user_id;
      UPDATE public.profiles SET coin_balance = coin_balance + 10, referred_by = referrer_user_id WHERE user_id = NEW.id;
